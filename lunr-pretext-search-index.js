...
},
{
  ...
  "type": "Section",
  "number": "3.2",
  "title": "Specific Final Project Requirements",
  "body": " Specific Final Project Requirements  The final project will utilize the Circuit Playground (or any other comparable or better microcontroller such as the Arduino or Raspberry Pi) and at least 3 other electrical components to move a mechanical system that helps people in some way. The specific requirements are shown below. Note that every group will have a few additional requirements specific to their project based on how creative they decide to be.  System must incorporate 3 electrical components (accelerometer, lights, sound and microphone to name a few). Your power source, microcontroller, speaker amplifier or the Crikit does not count as a component. An interface is not considered a component  System must be unique as well as built and designed by your team (Tutorials may be used but you must make the system unique in some way).  System must have one of the three components as an external part to the main microcontroller and not be something already taught in a weekly project (buttons, servos or potentiometers to name a few).  System must take some sort of measurement to be used in the code.  System must utilize a data acquisition method so that data can be plotted on your computer as a function of time.  System must help someone in some way.  System must have some sort of mechanical component to it and cannot be purely software and electrical components.    Microcontroller Choice: Note, you have the choice to use any type of microcontroller (MCU) for your final project. In my experience teaching this course, many groups choose to use an Arduino UNO\/Mega\/Nano or ESP32 for their final project. Although the CPX\/CPB is a cheap MCU that has many built-in sensors, it is severely lacking in stability, memory and power capabilities. The course is designed around CircuitPython and the CPX\/CPB due to the simplicity of the programming language and versatility of the MCU. However, for your final project, using an Arduino will open your project up to a larger world of possibilities and your CPX\/CPB programming\/hardware skills will transfer very easily to Arduino. Combine that with GenAI and you’ll have an easy time getting code to work on the Arduino. I would say about half the class ends up using an Arduino so although you can absolutely make some very cool projects with the CPX\/CPB, my experience is that about 50% of the class would benefit from using an Arduino. Why isn’t the entire class just built around Arduino? Because for $20 you get two push buttons, a slide switch, an accelerometer, a temperature sensor, 8 neopixels, PWM support, a light sensor, multiple 16 bit ADCs, a microphone, a speaker and a much simpler programming language. The CPX also has an IR sensor and the CPB supports bluetooth communication. I think students get more out of learning on a CPX\/CPB and then they can take those skills to something more reliable like the Arduino MCU or even something more complex like a Raspberry Pi.  "
},
{
  "id": "requirements-3",
  "level": "2",
  "url": "requirements.html#requirements-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Microcontroller Choice: "
},
{
  "id": "project-examples",
  "level": "1",
  ...
  "body": " Project Examples  The project schedule and requirements previously shared is a general framework that can be applied to any project and has been for many many semesters. When first formulating this course I came up with a few project ideas for students to use as inspiration but of course the Adafruit Learning System provides a wealth of resources for project development . Still, here are a few examples that are shown below that can still be used as inspiration for future students. I've also included a few examples of projects that students have built in the past to give you an idea of what is possible with the CPX and a little creativity.  A light sensor to turn a servo and then external leds  Pressure plate outside that sends a text when package is stolen  Log weather data on your wifi network with a raspberry pi and Adafruit IO  Use a computer fan you own to create a solder station filter  The examples below are from peers that have taken the class in the past  Little Robot with Adafruit Crickit  Automatic trash can - Uses an ultrasonic sensor and a servo to open a trash can  Light saber - an LED strip is used  Automatic shades - a light sensor is used to turn a relay on which runs a dc motor to open your blinds  Auto plant watering device - uses a waterproof pump and a humidity sensor to water a plant  Auto dog bowl water feeder - an ultrasonic sensor is used to detect water level and open a solenoid valve to let water flow into a bowl  Mobile for a child - a rotating platform with toys is attached to a continuous servo  Laser Tag - Two CPXes are used to play laser tag by sending and receiving IR signals  Mailbox - When mail is delivered light enters from the box from opening the mail box. This causes the CPX to send an IR signal to another CPX with says “You’ve got mail” with the built in speaker  Door Opener with Dog Bark - using a 5V relay and a large motor a door open when a sound byte over a certain value is read using a microphone  Shade Sense - Open blinds at a particular time or when sun light is bright enough using a light sensor and a servo  Security Box - The CPX is used to rotate a “lock” using a servo by pressing buttons in a predetermined sequence  Underglow that Bounces to Music - An LED strip is used that changes color by using the microphone to detect sound levels  Auto-soap dispenser - A servo presses down on soap when a hand is placed under an ultrasonic sensor.  Remote control car - A Circuit Playground Bluefruit (CPB) and Cricket are used to control a remote control car. Continuous servos are used to control the rear wheels and a front servo is used to control the front linkage. The CPB has bluetooth to allow control of the car using Bluetooth and an app on your phone.  Fortune teller - Student bought a coin machine so you could put a quarter in and it would then tell a fortune by rotating one of 4 servos.  Putt putt \/ skeeball \/ miniature bowling - 2 sonar sensors can be used to get speed of the ball  Tattle tale robot - Students used a motion sensor to detect when someone walked in the room    "
},
{
  "id": "pitot-probe-example",
  "level": "1",
  "url": "pitot-probe-example.html",
  "type": "Section",
  "number": "3.4",
  "title": "Pitot Probe Example",
  "body": " Pitot Probe Example   One example project that I will use to explain future diagrams is from a project I built a few years ago. The overall mission goal was to use a pitot probe to measure the velocity of my bicycle while riding around town . My initial hand sketch showed the pitot probe on the front of my bike which would use the pressure difference between the static and dynamic ports to calculate velocity (See ). The project was successful and I was able to get some good data on my bike rides around town. The project also included a mechanical subsystem which was a simple plastic case for the electronics and zip ties to mount the pitot probe as well as a data acquisition system using the CPB and a Bluetooth module to send the data to my phone in real time. The figure below showed my initial sketch for the project.   Initial Sketch of Pitot Probe Project  After the sketch it's important to be sure that all requirements set by the instructor are satisfied. In this case I needed 3 components so I decided to use the bluetooth transmitter, pitot probe, neopixels and button on the CPB which ended up being 4 components but I thought it would be nice to have visual feedback so I added the neopixels. The pitot probe would be the external component and the button would be used to calibrate the pitot probe. Finally, the bluetooth would be used to send the data to my phone. Although the course has a set of general requirements defined in , I still needed to add a few specific requirements as shown below.  The system shall measure the windspeed of the vehicle.  The system shall be calibrated on startup or during operation by pressing a button.  The system will be powered during operation of the vehicle.  The system shall not adversely affect the vehicle performance.  The system shall send windspeed data to the operators smart phone.  The system shall light up during calibration to indicate to the user that the system shall not be touched.  Note however that I failed to satisfy the requirement of System must have one of the three components as an external part to the main microcontroller and not be something already taught in a weekly project . This is because the pitot probe project is a weekly project (See ). However, I still think this is a good example of how to use the systems engineering life cycle to design an electrial subsystem so I will continue with this example.    Diagrams  Upon completion of the initial sketch and the requirements it is now time to move away from a mission goal with requirements and begin the conceptual design process. This requires using the requirements to figure out what components and modules are necessary to accomplish the system you are trying to build. In order to do this, many diagrams are going to be created namely Block Definition Diagrams (BDDs) and Internal Block Diagrams (IBDs). I am going to be using DrawIO to create these diagrams .    Electrical Subsystem  Block Definition Diagrams (BDD) are used to show the different components and modules of a system and how they are related to each other. For the pitot probe project, the electrical BDD is the easiest and most intuitive to complete first and is shown below.   BDD of Electrical Components  The BDD shows the different components of the system which includes general names for all subsystems included. Notice that instead of writing pitot probe specifically I wrote Windspeed Sensor . This was I can elect to change to something different later on in the design process if I decide to do a last minute design change. Once you have the BDD completed it is time to move on to the Internal Block Diagram (IBD) which shows how the different components are connected to each other. The IBD for the electrical subsystem is shown below.   IBD of Electrical Components  The IBD shows how the different components are connected to each other. In this case, the Windspeed Sensor is connected to the CPU, the Notification UI(most likely neopixels) is connected to the CPU as well as the Calibration UI (most likely a button) and the CPU will send data to the Wireless Communications module (most likely bluetooth). The IBD also shows the flow of data between the different components and using annotations via text boxes also shows what information is passed from one block to the next. Notice also that every block shown in the BDD is in the IBD except for the central block which is merely the name of the system. Finally, I want to draw attention to the fact that I do not have a data storage module because all data will be sent to my smart phone and logged there which satisfies one of my major requirements. The IBD is an important step in the design process as it helps to ensure that all components are properly connected and that the flow of data is correct. It also helps to identify any potential issues with the design before moving on to the detailed design phase.    Software Subsystem  Once you have completed the electrical BDD and IBD the software IBD is almost identical. The only difference is you need to first identify which components require software. For example, the power supply does not require any software but the wireless communication module does. The software BDD is shown below.    BDD of Software Components   Notice that every software name contains () to indicate that it will eventually be a function. This is useful at this stage to help later on in the preliminary design phase. The CPU requires much software to run the system but since we're programming everything in a very high level programming language like Python, CircuitPython or Arduino, we're assuming that the only software required by the user is just a main loop. In Python\/CircuitPython the main loop is typically just a while True: loop and in Arduino it is the loop() function. Once the BDD is completed, the blocks in the BDD can be used to create the IBD which is shown below.    IBD of Software Components   Again, The flow of data is similar to the electrical IBD but now we can see what information is being passed from one function to the next which again will be helpful when we get to the preliminary design phase.    Mechanical Subsystem  The mechanical subsystem is a bit more difficult to create BDDs and IBDs for mostly because of how trivial and simple the systems are. Typically these diagrams are so simple that students often get confused and over complicate these diagrams. However, if you look at the diagrams below it's easy to see that mechanical system is important but much smaller and less complex than the electrical and software subsystems. The BDD for the mechanical subsystem is shown below.   BDD of Mechanical Components  In this case my plan was to attach the windspeed sensor to my handelbars somehow and then put all the other electrical components into a main houseing. Then all I would have to do is figure out how to run the wires from the main housing to the windspeed sensor. So the BDD only has 3 components. With the BDD completed, I moved on to the mechanical IBD which is shown below.   IBD of Mechanical Components  In this case I elected to add the bicycle to the IBD because even though the bicycle is an interface I still wanted to show how the system would be attached to the bicycle and how the different components would be connected to it. The IBD shows that the main housing is attached to the bicycle via velcro strap and the windspeed sensor is attached to the bicycle via zip ties.    Preliminary Diagrams  Following the conceptual design phase where missions goals, requirements, hand sketches, BDDs and IBDs are created, it is time to work out component selection. For example, instead of calling the module Windspeed Sensor it is time to select the component and call it a pitot probe. Similarly, buttons, neopixels and bluetooth must be selected. In the preliminary design phase the mechanical lead must create a simple layout of the system. This requires a simple top down layout of the system with dimensions to give the designer an idea of how that's going to go together. For this, a simple sketchup tool can be used like Inkscape . Because my project was so simple I did not create a simple layout for this system. For the electrical lead, a connections diagram is necessary to show how the different components are connected to each other. This would be similar to the electrical IBD except this would be actual components with references to actual parts. For this project the connections diagram is super simple because the CPU, clock, bluetooth module, buttons, and the neopixels are all built into the CPB so the only external component is the pitot probe which is connected to the CPB. So my connections diagram just has 3 components: the CPB, pitot probe and power supply.   Connections Diagram for Bike Project  For the software lead it is important to create psuedo code in the form of a software flow chart. These charts are something that is often overlooked but can be very helpful in the design process. The flowchart created for this pitot probe bike project is shown below.   Psuedo Code Flow Chart for Bike Project  In this flow chart the code starts by initializing all the different modules, then turns on the neopixels. After that it checks if the bluetooth is connected. If it is not connected, then the neopixels turn blue and the system waits until a connection is made. Once a connection is made, the system checks if the button is pressed. If the button is pressed, then the system goes into calibration mode where it takes 100 samples of the voltage and averages them to get a bias voltage which is used to calibrate the system. During calibration, the neopixels turn green to indicate to the user that they should not touch the system. If the button is not pressed, then the system takes 10 samples of the voltage and averages them to get an average voltage which is then converted to windspeed using a conversion function. Finally, the data is sent to the user's phone via bluetooth and also printed to serial for debugging purposes.    Detailed Design  Once the conceptual and preliminary design phase is over it is time to create the detailed design. The detailed design needs to be so detailed that anyone following the design on paper to turn the system into a reality just by reading all of the schematics. Thus, for the mechanical lead, some sort of CAD (Computer Aided Design) tool is necessary for dimensioned drawings and detailed schematics. In this case I recommend using your favourite CAD program such as Blender , SolidWorks , Fusion 360 or anything you're comfortable with . For the electrical lead on the team, wiring diagrams are necessary and I recommend using either CirKit Designer or Fritzing . CAD and wiring diagram examples will not be shown in this chapter simply because numerous examples of wiring diagrams are shown in virtually every weekly project and 3D CAD is the subject of an entire course in and of itself.  For the software lead it is time to figure out what software is required to run each component. For example, the pitot probe as explained in is an analog sensor which means you need the analogio module. The neopixels need the neopixel . This process must be repeated for every component to be sure that you have all the modules necessary to run the program. Although this is a lot of work it helps tremendously later on in the subsystem testing phase. In this case the list below contains all the modules I need to run this system .  time - for the clock  analogio - for the pitot probe  adafruit_ble - for the bluetooth connection  digitalio - for the button  neopixel - for the neopixels  CircuitPython - for the main programming language  Notice that I didn't need anything for the battery since that's just hardware. An interesting corollary is that it's actually possible to create a software flow chart that is so good that at the time of this writing if you upload the screenshot of the psuedo code flowchart to Gemini it will give you the actual code for it in any language you like . In the youtube tutorial discussed in , I show how to do this but for the sake of this book I will show the code that was generated from the flow chart. import math import time import board import analogio import digitalio import adafruit_ble from adafruit_ble.advertising.standard import SolitaryAdvertisement from adafruit_led_animation.animation.solid import Solid import neopixel # --- Configuration & Initialization --- rho = 1.225 v_bias = 0.0 # Initial calibration value # Setup Analog Input (Change 'A1' to your sensor pin) airspeed_sensor = analogio.AnalogIn(board.A1) # Setup Button A button = digitalio.DigitalInOut(board.BUTTON_A) button.direction = digitalio.Direction.INPUT button.pull = digitalio.Pull.DOWN # Adjust to Pull.UP if your button logic is inverted # Setup NeoPixels pixels = neopixel.NeoPixel(board.NEOPIXEL, 10, brightness=0.2) RED = (255, 0, 0) BLUE = (0, 0, 255) GREEN = (0, 255, 0) OFF = (0, 0, 0) # Setup Bluetooth ble = adafruit_ble.BLERadio() advertisement = SolitaryAdvertisement() def get_voltage(): # Helper to convert 16-bit ADC value to Voltage (0-3.3V) return (airspeed_sensor.value * 3.3) \/ 65535 def get_wind_data(samples): \"\"\"Measures analog value, converts to voltage, and averages.\"\"\" total_voltage = 0 for _ in range(samples): total_voltage += get_voltage() return total_voltage \/ samples def convert_2_windspeed(avg_voltage): \"\"\"Converts voltage to windspeed using dP = 1000 * dV.\"\"\" dv = avg_voltage - v_bias dp = 1000 * dv # Ensure dp is not negative before square root if dp < 0: dp = 0 windspeed = math.sqrt((2 * dp) \/ rho) return windspeed def neo_pixel_run(state, color=OFF): \"\"\"Sets the NeoPixel state and color.\"\"\" if state: pixels.fill(color) else: pixels.fill(OFF) pixels.show() # --- Main Loop --- print(\"System Initialized...\") while True: # Bluetooth Connection Check if not ble.connected: neo_pixel_run(True, BLUE) if not ble.advertising: ble.start_advertising(advertisement) # Flowchart shows loop back to start\/check if disconnected continue # Check Button Routine if button.value: # Calibration Branch neo_pixel_run(True, GREEN) avg_v = get_wind_data(100) v_bias = avg_v # Calibrate() routine print(f\"Calibrated! New V_Bias: {v_bias}\") else: # Standard Measurement Branch neo_pixel_run(False) avg_v = get_wind_data(10) speed = convert_2_windspeed(avg_v) current_time = time.monotonic() # GetTime() # SendData() routine (Printing to Serial\/BLE) data_string = f\"Time: {current_time:.2f}s | Windspeed: {speed:.2f} m\/s\" print(data_string) # If you have a specific BLE UART service, you'd send data_string there. You can see from the code generated that with the help of creating a psuedo code flowchart and the power of AI you can create code from scratch that actually flows the way you want it to. Note that the code above was not tested and may not actually work the way you intended. However, just by glancing at it, it is a great start and shows the power of creating this detailed flow chart. Just to be thorough, the code I ended up using is on Github and you can see how similar the code is between the autogenerated code and my code that I built from scratch before AI was widely adopted .    Critical Design Review (CDR)  After the detailed design, the Critical Design Review (CDR) is conducted to evaluate the design against its requirements. This involves a live presentation to stakeholders and peers. In this presentation the students are required to explain their current design and respond to any quesitons from the audience. This is a good way to receive feedback about the project before moving into subsystem testing and final verification and validation. Of course, for my project I did not do a CDR and moved directly into subsystem testing.    Subsystem Testing  For my first test, I elected to deal with the pitot probe and used Method 1 ( ) to record data with my laptop. Then I had my parents drive down the highway while I hung the pitot probe out the window. I was able to get a nice plot of windspeed. One issue I ran into was that when the car came to a stop the speed did not. I realized I needed to go ahead and add the button press so that I could calibrate often. Hence why having the button press was so crucial. Once I was sure the pitot probe worked and my button was able to calibrate I added the bluetooth module and repeated the same experiment only this time I plotted the data on my phone as shown below.   Plot of Windspeed Data During Calibration Test  Once I knew the button, pitot probe and bluetooth were working I added the neopixels and tested that as well although I do not have a photo of that particular test.    Test Readiness Review (TRR) and Systems Verification Review (SVR)  Once I tested all the components it was time to integrate everything together by placing everything into a small case on my bike. I then took the system on a bike ride around town and was able to get some good data. The final product is shown below.   Main Housing of Pitot Probe Bike Project  Overall the prototype worked as intended. It recorded wind speed and had a button to re-calibrate. While the system is recalibrating the neopixels light up to notify the user not to start biking yet. The housing mounted to the top tube of my bike as shown in the figure above and I ended up using zip ties to secure the pitot tube to the front of my bike like in the photo below.   Final Product with Pitot Probe Mounted to Front of Bike  I have future ideas where I could upgrade the power supply and even install a waterproof toggle switch so that I can turn the device on and off without having to remove the entire housing. It would also be nice to have a button on the outside to recalibrate without having to remove the entire housing. Still, all requirements were satisfied and the project was a success as shown in the table below. Note that satisfied requirements are in plain text, partially satisfied comments are in italics and bold text are requirements that were not satisfied. In this case, all requirements were satisfied except for 1 which was partially satisfied.   System Requirements Verification Matrix      Requirement  Verification \/ Status    The system shall measure the windspeed of the vehicle  Yes, with a pitot probe    The system shall be calibrated on startup or during operation by pressing a button  Yes, on start up and by pressing button A    The system will be powered during operation of the vehicle  Yes, with a portable battery pack    The system shall not adversely affect the vehicle performance.  Yes, the user did not report any issues while biking.    The system shall send windspeed data to the operators smart phone.  Yes, with bluetooth    The system shall light up during calibration to indicate to the user that the system shall not be touched.  Yes, it lights up green    System must incorporate 3 electrical components  System had four: pitot probe, neopixels, button, and bluetooth.    System must be unique as well as built and designed by your team  Yes, I created the entire project from scratch and only grabbed inspiration from other projects.    System must have one of the three components as an external part to the main microcontroller and not be something already taught in a weekly project.  Partially, the pitot probe was external but already a part of the weekly projects.    System must take some sort of measurement to be used in the code  Yes, it took time and windspeed data.    System must utilize a data acquisition method so that data can be plotted on your computer as a function of time  Yes, Method 5 was utilized    System must help someone in some way  Yes, it allowed the user to ascertain its wind windspeed.    System must have some sort of mechanical component to it and cannot be purely software and electrical components.  Yes, it had a main housing and pitot probe zip tied to the handlebars.     This is just one example of how to use the systems engineering life cycle to design and build a project. The same process can be applied to any project and can be used to help ensure that the project is successful. The key is to follow the process and not skip any steps. By following the process, you can ensure that your project is well designed, well built and meets all of its requirements. Hopefully this example gets you excited about the CPX\/CPB and the entire Adafruit ecosystem as well as a quick preview of what they can do. Hopefully you enjoy the following chapters which details individual weekly projects geared to help you learn simple sensors and electrical components that you can piece together and build your own DIY electro-mechanical system.   "
},
{
  "id": "pitot-sketch",
  "level": "2",
  "url": "pitot-probe-example.html#pitot-sketch",
  "type": "Figure",
  "number": "3.4.1",
  "title": "",
  "body": "  Initial Sketch of Pitot Probe Project  "
},
{
  "id": "bdd-electrical",
  "level": "2",
  "url": "pitot-probe-example.html#bdd-electrical",
  "type": "Figure",
  "number": "3.4.2",
  "title": "",
  "body": "  BDD of Electrical Components  "
},
{
  "id": "ibd-electrical",
  "level": "2",
  "url": "pitot-probe-example.html#ibd-electrical",
  "type": "Figure",
  "number": "3.4.3",
  "title": "",
  "body": "  IBD of Electrical Components  "
},
{
  "id": "bdd-software",
  "level": "2",
  "url": "pitot-probe-example.html#bdd-software",
  "type": "Figure",
  "number": "3.4.4",
  "title": "",
  "body": "  BDD of Software Components  "
},
{
  "id": "ibd-software",
  "level": "2",
  "url": "pitot-probe-example.html#ibd-software",
  "type": "Figure",
  "number": "3.4.5",
  "title": "",
  "body": "  IBD of Software Components  "
},
{
  "id": "bdd-mechanical",
  "level": "2",
  "url": "pitot-probe-example.html#bdd-mechanical",
  "type": "Figure",
  "number": "3.4.6",
  "title": "",
  "body": "  BDD of Mechanical Components  "
},
{
  "id": "ibd-mechanical",
  "level": "2",
  "url": "pitot-probe-example.html#ibd-mechanical",
  "type": "Figure",
  "number": "3.4.7",
  "title": "",
  "body": "  IBD of Mechanical Components  "
},
{
  "id": "connections-diagram",
  "level": "2",
  "url": "pitot-probe-example.html#connections-diagram",
  "type": "Figure",
  "number": "3.4.8",
  "title": "",
  "body": "  Connections Diagram for Bike Project  "
},
{
  "id": "pitot-psuedo-code",
  "level": "2",
  "url": "pitot-probe-example.html#pitot-psuedo-code",
  "type": "Figure",
  "number": "3.4.9",
  "title": "",
  "body": "  Psuedo Code Flow Chart for Bike Project  "
},
{
  "id": "calibration-plot",
  "level": "2",
  "url": "pitot-probe-example.html#calibration-plot",
  "type": "Figure",
  "number": "3.4.10",
  "title": "",
  "body": "  Plot of Windspeed Data During Calibration Test  "
},
{
  "id": "final-product",
  "level": "2",
  "url": "pitot-probe-example.html#final-product",
  "type": "Figure",
  "number": "3.4.11",
  "title": "",
  "body": "  Main Housing of Pitot Probe Bike Project  "
},
{
  "id": "final-product-2",
  "level": "2",
  "url": "pitot-probe-example.html#final-product-2",
  "type": "Figure",
  "number": "3.4.12",
  "title": "",
  "body": "  Final Product with Pitot Probe Mounted to Front of Bike  "
},
{
  "id": "table-system-verification",
  "level": "2",
  "url": "pitot-probe-example.html#table-system-verification",
  "type": "Table",
  "number": "3.4.13",
  "title": "System Requirements Verification Matrix",
  "body": " System Requirements Verification Matrix      Requirement  Verification \/ Status    The system shall measure the windspeed of the vehicle  Yes, with a pitot probe    The system shall be calibrated on startup or during operation by pressing a button  Yes, on start up and by pressing button A    The system will be powered during operation of the vehicle  Yes, with a portable battery pack    The system shall not adversely affect the vehicle performance.  Yes, the user did not report any issues while biking.    The system shall send windspeed data to the operators smart phone.  Yes, with bluetooth    The system shall light up during calibration to indicate to the user that the system shall not be touched.  Yes, it lights up green    System must incorporate 3 electrical components  System had four: pitot probe, neopixels, button, and bluetooth.    System must be unique as well as built and designed by your team  Yes, I created the entire project from scratch and only grabbed inspiration from other projects.    System must have one of the three components as an external part to the main microcontroller and not be something already taught in a weekly project.  Partially, the pitot probe was external but already a part of the weekly projects.    System must take some sort of measurement to be used in the code  Yes, it took time and windspeed data.    System must utilize a data acquisition method so that data can be plotted on your computer as a function of time  Yes, Method 5 was utilized    System must help someone in some way  Yes, it allowed the user to ascertain its wind windspeed.    System must have some sort of mechanical component to it and cannot be purely software and electrical components.  Yes, it had a main housing and pitot probe zip tied to the handlebars.    "
},
{
  "id": "sys-eng-extra-help",
  "level": "1",
  "url": "sys-eng-extra-help.html",
  "type": "Section",
  "number": "3.5",
  "title": "Extra Help",
  "body": " Extra Help  I have a youtube tutorial on how to create an airplane in Blender which you could easily adapt to anything else if you're having trouble making 3D objects. I similarly, have a tutorial on creating BDDs and IBDs in DrawIO I also have a youtube tutorial where I create a psuedo code flow chart just like the chart in this chapter. Finally, I have another youtube tutorial on how to make a simple circuit in Cirkit Designer .  "
},
{
...
},
{
  ...
  "number": "4.3",
  "title": "Extra Help",
  "body": " Extra Help  I created an unboxing video on Youtube for you to take a look and see the actual hardware as I talk about each piece of equipment .  "
},
{
  "id": "purchase-equipment-6",
  "level": "1",
  "url": "purchase-equipment-6.html",
  "type": "Section",
  "number": "4.4",
  "title": "Assignment",
  "body": " Assignment   Your assignment for this module is to purchase the required equipment for your class. Note that the specific equipment will be instructor dependent so be sure you discuss with the instructor of your course before buying everything. At a minimum you must purchase a CPX or CPB but the rest of the items in your kit will depend on which modules your instructor wants you to do during the semester. Also, the CPX\/CPB requires a microUSB cable and that cable must have a data line. Many USB cables are just power and ground.    Title page with name,title and date - 5%  Introduction - 5%  All figures must have appropriate annotations, figure captions, figure labels and a paragraph of supporting text - 10%  A figure of your receipt of your purchases this semester (If you already own the MCU you may just take a photo of you holding the device). - 60%  Using the number of students in your class, compute the probability that at least one MCU will be shipped DOA (dead on arrival) assuming a low (p=1%) failure rate. - 20%  Appendix A - Video Details - Use the webcam on your computer to film yourself introducing yourself to me and either show me your purchased MCU or film your screen and show the receipt that shows you've purchased the MCU. - Pass\/Fail  Appendix B - Code used to take data and code used to plot in plain text (no screenshots) - Pass\/Fail  Appendix C - Screenshot of Excel or Sheets if applicable - Pass\/Fail  If you are asked to fix something in the Appendices such as missing content in your video or screenshots of your code, you will lose 10% of your grade everytime you submit an update after the deadline.    "
},
{
  "id": "plotting-3",
  "level": "1",
  ...
  "number": "5.1",
  "title": "Python IDEs",
  "body": " Python IDEs   So which IDE is the best and which is recommended? Two IDEs are recommended for Python. They are explained in the next two sections. Either one is totally fine. If you just Google Python download you will find a humongous list of editors (Scratch, Anaconda, Canopy, Eclipse, PyDev, etc). It's easy to get lost when searching for something so broad. Again, just using Spyder or Thonny.    Thonny  Thonny - https:\/\/thonny.org is a great, easy, simple and lightweight IDE that has been around for years. Note that there is a 32 bit and 64 bit version. Which one is necessary? Well that depends on how many bits the computer has. This is a great thing to Google. Type the following: \"do I have a 32 bit or 64 bit computer\" into Google. It's more than likely that everyone reading this has a 64 bit computer but it's always a good idea to check. Learning the difference between 32 and 64 bit computers is the topic of another chapter ( ). The figure below is a screenshot of what Thonny looks like with a sine graph plotted.    Example Sine Graph in Thonny     Spyder  Spyder - https:\/\/www.spyder-ide.org\/ is a powerful IDE that is widely used in the scientific Python community. The figure below is a screenshot of what Spyder looks like with a sine graph plotted.    Example Sine Graph in Spyder     Setting up the IDE  Once Thonny or Spyder is installed, numpy and matplotlib are necessary modules within Python that allow Python to do some extra things like numerical computation with Python (numpy) and Matlab style Plotting libraries (matplotlib). How to install modules in Youtube is in the Extra Help section ; however, the simple way is to head over to Tools>Manage Packages in Thonny. The image below shows that version 3.1.2 is already installed but can be upgraded to 3.2.2    Installing Matplotlib in Thonny   If numpy or matplotlib is not already included in Spyder then type the following into the Python Console in the lower right hand corner of Spyder which is called the IPython console.  !pip install matplotlib  If that doesn't work try  !pip3 install matplotlib  The output example below shows that matplotlib is already installed installed as it says requirement already satisfied . Assuming there is a valid internet connection it will install the necessary module.    Installing Python Modules via Thonny Console     Scripting  Once numpy and matplotlib is installed it's time to make a plot. Here is a simple example.    Close up text of plotting in Python   The code above will plot a sine wave from 0 to 2pi. The two lines at the top are importing the numpy and matplotlib modules you installed earlier. When they are imported we give them shorter names so it's easier to reference them so numpy will now be called np and matplotlib.pyplot will be called plt. The next two lines then create a vector x from 0 to 2pi using 1000 data points. The next line then uses the sine function to create the vector y . Finally x and y are plotted and the figure is instructed to pop up on your screen using the show() function.   "
},
{
  "id": "plotting-3-3-3",
  "level": "2",
  "url": "plotting-3.html#plotting-3-3-3",
  "type": "Figure",
  "number": "5.1.1",
  "title": "",
...
},
{
  "id": "plotting-3-4-3",
  "level": "2",
  "url": "plotting-3.html#plotting-3-4-3",
  "type": "Figure",
  "number": "5.1.2",
  "title": "",
...
},
{
  "id": "plotting-3-5-3",
  "level": "2",
  "url": "plotting-3.html#plotting-3-5-3",
  "type": "Figure",
  "number": "5.1.3",
  "title": "",
...
},
{
  "id": "plotting-3-5-9",
  "level": "2",
  "url": "plotting-3.html#plotting-3-5-9",
  "type": "Figure",
  "number": "5.1.4",
  "title": "",
...
},
{
  "id": "plotting-3-6-3",
  "level": "2",
  "url": "plotting-3.html#plotting-3-6-3",
  "type": "Figure",
  "number": "5.1.5",
  "title": "",
...
},
{
  ...
  "type": "Section",
  "number": "5.2",
  "title": "Other Plotting Tools",
  "body": " Other Plotting Tools   Google Colab  It is possible to use Google Colab to collaborate on Python projects, run in the cloud\/browser. In order to get Colab to work a Google account is necessary. Colab is a great tool for collaborating on Python projects and sharing code with others. It also has the advantage of running in the cloud so it can be accessed from any device with an internet connection. Although it's possible to head directly to the Colab website, it is recommended to open Colab through Google Drive. To do that simply open Google Drive>+New>More>Connect more apps and then install Google Colab as shown in the figure below.    Installing Google Colab through Google Drive   Once that's done it will be possible to open a new Google Colab notebook. The figure below is a screenshot of what Colab looks like with a sine graph plotted. The other benefit to using Google Colab is that Gemini is imbedded into Google Colab for easy AI integration.    Example Sine Graph in Google Colab     Octave  Octave is very similar to MATLAB but free. The only major downside is that Octave is much slower and doesn't have a lot of extra utilities that MATLAB does. However, if it's not possible to get MATLAB, Octave is a great alternative. The figure below is a screenshot of what Octave looks like with a sine graph plotted.    Example Sine Graph in Octave   Notice that the syntax is very similar to Python but slightly different. Deep diving into programming is not the focus of this class but anyone wanting to learn more about programming in Python or Octave can find plenty of resources online. The best way to learn is to just start writing code and then Google any errors that come up. The error messages are usually pretty descriptive and will lead to the answer. Furthermore, with the spawn of Generative AI many students often code better than ever before with a few prompts into any AI tool like Gemini, Copilot, Grok or ChatGPT. Also, the extra help section (xref ref=\"extra-help\"\/>) has many youtube videos on how to program if videos are preferred to AI or text.    Google Sheets  Google Sheets is typically already installed in all browsers again provided a Google account is created first. However, Microsoft Excel can also be used if a Google account is not available. The figure below is a screenshot of what Google Sheets looks like with a sine graph plotted. Plotting in spreadsheet programs can be cumbersome but it is possible to do and is a good option for those who are not interested in learning Python or Octave or programming in general. Futhermore, there is a tongue-in-cheek comment from recently graduated engineers that no one knows how to code and everyone just uses Excel. So if that's the future of engineering, Google Sheets or Excel is the way to go.    Example Sine Graph in Google Sheets    "
},
{
  "id": "plotting-4-2-3",
  "level": "2",
  "url": "plotting-4.html#plotting-4-2-3",
  "type": "Figure",
  "number": "5.2.1",
  "title": "",
  "body": "  Installing Google Colab through Google Drive  "
},
{
  "id": "plotting-4-2-5",
  "level": "2",
  "url": "plotting-4.html#plotting-4-2-5",
  "type": "Figure",
  "number": "5.2.2",
  "title": "",
  "body": "  Example Sine Graph in Google Colab  "
},
{
  "id": "plotting-4-3-3",
  "level": "2",
  "url": "plotting-4.html#plotting-4-3-3",
  "type": "Figure",
  "number": "5.2.3",
  "title": "",
  "body": "  Example Sine Graph in Octave  "
},
{
  "id": "plotting-4-4-3",
  "level": "2",
  "url": "plotting-4.html#plotting-4-4-3",
  "type": "Figure",
  "number": "5.2.4",
  "title": "",
  "body": "  Example Sine Graph in Google Sheets  "
},
{
  "id": "extra-help",
  "level": "1",
...
},
{
  "id": "extra-help-3-10",
  "level": "2",
  "url": "extra-help.html#extra-help-3-10",
  "type": "Figure",
  "number": "5.3.2",
  "title": "",
...
},
{
  ...
  "number": "5.4",
  "title": "Assignment",
  "body": " Assignment   The assignment for this project is to plot the equation below from 0 to 10 seconds and include that Figure in a report. The Figure must include a grid and an x-axis label (‘Time (sec)') and y-axis label (‘Temperature (F)'). Any plotting tool Sheets, Excel, Colab, Python, Octave or similar is acceptable provided the figure conforms to the requirements above and the specific rubric below.   Title page with name,title and date - 5%  Introduction - 5%  All figures must have appropriate annotations, figure captions, figure labels and a paragraph of supporting text - 10%  Include a screenshot (entire screen) of your plotting environment with your code pasted into the workspace - 40%  Include the plot of temperature vs time being sure to save the figure so it is in high resolution - 40%  Appendix A - Video Details - Film yourself and your entire screen running the example code and showing the result of plt.show() - Pass\/Fail  Appendix B - Code used to take data and code used to plot in plain text (no screenshots) - Pass\/Fail  Appendix C - Screenshot of Excel or Sheets if applicable - Pass\/Fail  If you are asked to fix something in the Appendices such as missing content in your video or screenshots of your code, you will lose 10% of your grade everytime you submit an update after the deadline.     "
},
{
...
{
  "id": "getting-started-setup-5",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-5",
  "type": "Figure",
  "number": "6.1.2",
  "title": "",
...
{
  "id": "getting-started-setup-7",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-7",
  "type": "Figure",
  "number": "6.1.3",
  "title": "",
...
{
  "id": "getting-started-setup-12",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-12",
  "type": "Figure",
  "number": "6.1.4",
  "title": "",
...
{
  "id": "getting-started-setup-15",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-15",
  "type": "Figure",
  "number": "6.1.5",
  "title": "",
...
{
  "id": "getting-started-setup-19",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-19",
  "type": "Figure",
  "number": "6.1.6",
  "title": "",
...
{
  "id": "getting-started-setup-21",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-21",
  "type": "Figure",
  "number": "6.1.7",
  "title": "",
...
{
  "id": "getting-started-setup-26",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-26",
  "type": "Figure",
  "number": "6.1.8",
  "title": "",
...
{
  "id": "getting-started-setup-28",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-28",
  "type": "Figure",
  "number": "6.1.9",
  "title": "",
...
{
  "id": "getting-started-setup-32",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-32",
  "type": "Figure",
  "number": "6.1.10",
  "title": "",
...
{
  "id": "getting-started-setup-35",
  "level": "2",
  "url": "getting-started-setup.html#getting-started-setup-35",
  "type": "Figure",
  "number": "6.1.11",
  "title": "",
...
},
{
  ...
  "type": "Section",
  "number": "6.2",
  "title": "TL;DR \/ Extra Help for the CPX\/CPB",
  "body": " TL;DR \/ Extra Help for the CPX\/CPB   Plug in your CPX and double tap the reset button to go into reset mode. CPLAYBOOT will mount to your computer. (Some CPXes you need to hold the button down instead of double tap).  Download the UF2 file .  Drag the UF2 file to your CPLAYBOOT drive. After a few seconds CIRCUITPY will mount.  You need to then download Mu .  Open Mu and make sure to select the mode Adafruit CircuitPython (or just CircuitPython if you have a new version)  Open main.py (or code.py) in Mu from the CIRCUITPY drive in Mu. (It's possible you have code.py on your drive instead of main.py which is fine. Just open one or the other)  Copy the blink.py script into main.py (or code.py)   There is an accompanying youtube video to help you see me perform the 8 steps above .  "
},
{
...
},
{
  ...
  "number": "6.4",
  "title": "Assignment",
  "body": " Assignment   For this assignment you must appropriately set up your CPX\/CPB, get the blink code to run and include some screenshots (your entire computer screen) of relevant steps. Note for the blink code, modify blink pattern to be different from the default on Github . The specific items requested are shown below.   Title page with name,title and date - 5%  Introduction - 5%  All figures must have appropriate annotations, figure captions, figure labels and a paragraph of supporting text - 10%  Include a screenshot (entire computer screen) of your computer showing the CPLAYBOOT drive on your computer - 20%  Include a screenshot (entire computer screen) of your computer showing the CIRCUITPY drive on your computer - 20%  Include a screenshot (entire computer screen) of Mu with the Serial monitor open showing the output of the blink code. Remember to modify the blink pattern - 20%  Include a photo of your CPX\/CPB with the red led on - 20%  Appendix A - Video Details - Film yourself showing the CPX\/CPB blinking and your entire screen with your blink code running with the Serial monitor open - Pass\/Fail  Appendix B - Code used on your CPX\/CPB and code used to plot in plain text (no screenshots) - Pass\/Fail  Appendix C - Screenshot of Excel or Sheets if applicable - Pass\/Fail  If you are asked to fix something in the Appendices such as missing content in your video or screenshots of your code, you will lose 10% of your grade everytime you submit an update after the deadline.    "
},
{
  ...
  "body": " Updating your Bootloader  Some of the newer CircuitPython UF2s do not install correctly on old boot loaders. If you put the CPX\/CPB into CPLAYBOOT mode and open the INFO_UF2.TXT file it will tell you the version of the bootloader that you have just like the figure below.   Old Bootloader Version  In this case the version of the bootloader is 0.2.11 which is older than most new UF2s require. If you're having an issue with an old bootloader be sure to update the bootloader for the CPB or alternatively the bootloader for the CPX . At the time of this writing here is the command you need to upload the new bootloader the CPB. If you have version 0.4.0 or later you can use the UF2 method  . If your bootloader is too old you need adafruit-nrfutil . Here is the command you need to upload the new bootloader to the CPB. adafruit-nrfutil --verbose dfu serial --package circuitplayground_nrf52840_bootloader-0.8.2_s140_6.1.1.zip -p \/dev\/ttyACM0 -b 115200 --singlebank --touch 1200  For windows users you use the adafruit-nrfutil.exe file from the Adafruit bootloader website. Then, usingcommand prompt to run the command above you need to change the tty port to the correct COM port which can be found using Device Manager .   For linux users you will need a few things  You need pip3 $ sudo apt install python3-pip  You need adafruit-nrfutil $ pip3 install --user adafruit-nrfutil  adafruit-nrfutil gets installed into \\textasciitilde\/.local\/bin  You need to add your user to dialout $ sudo usermod -a -G your_username  Restart your computer  This is what it looks like when running the adafruit-nrfutil command on linux.   Output from running the bootloader update command on linux  For the CPX there is a UF2 that you transfer over to the CPLAYBOOT drive. At the time of this writing the version you're looking for is update-bootloader-circuitplay_m0-v3.15.0.uf2 . Once you update the bootloader you can check the version by opening the INFO_UF2.TXT file again and it should show the new version. The figure below shows an example of the new bootloader version after updating. In this case the version is 0.10.0 which is much newer than the old 0.2.11 version.   Updated Bootloader Version    "
},
{
...
},
{
  ...
  "level": "1",
  "url": "installing-modules.html",
  "type": "Section",
  "number": "7.2",
  "title": "Installing Modules",
  "body": " Installing Modules   One issue you're going to run into when you run certain codes is that you won't have some of the modules on your CPB\/CPX. To fix this you need to download the CircuitPython Libraries . You need to download the appropriate version: 6.x, 7.x or 8.x. The figure below is a screenshot of the libraries homepage on the CircuitPython website.   CircuitPython Libraries homepage  How do you know what version of CircuitPython you have? Well head over to your CIRCUITPY drive and open the boot_out.txt file and it will tell you the version. The Figure below shows an example boot_out.txt file. In this case the version is 10.1.4 so, you would download the 10.x version of the libraries.   Example boot_out.txt file showing the version of CircuitPython  Note that this is the same version as the .UF2 file installed back in the Getting Started labs (See Chapter ). When you download the modules it will download a .zip file. Extract the .zip file on your desktop computer and then open the lib folder on your desktop and your CIRCUITPY. You then need to transfer the modules (ONLY THE ONES YOU NEED) from your desktop to your CPX\/CPB lib folder. The reason why you can't copy the entire folder is because the CPB\/CPX only has 2MB of flash and the CircuitPython download is 4.1 MB at the time of this writing. If your CIRCUITPY drive does not have a lib folder you can easily create one as shown in the Figure below.   CIRCUITPY Drive showing lib folder    After extracting the zip folder to your computer you can begin dragging files over to your CPX\/CPB. The Figure below shows an example lib folder full of modules that have been dragged from the desktop to the CIRCUITPY drive.   Example lib folder on CIRCUITPY drive with modules dragged over from desktop  The follow up question of course is Which modules do I need? The answer to that question is in the code. When you run a code that needs a specific module, the Serial monitor will say ImportError: No module named 'x' . In this case you will then know to install the module 'x'. You can also look at the import statements at the top of the code to see which modules are being imported and then make sure you have those modules installed. The figure below is an example of that error where the code is specifically looking for the module named adafruit_circuitplayground . In this case, the user would need to copy that specific module from the desktop to the CIRCUITPY drive lib folder.   Example of a missing module error  Note that it's possible to import too many modules on the CPX\/CPB and run out of memory. It turns out that the CPB has much more memory then the CPX. Run the commands below to see how much memory your CPX\/CPX has left. import gc gc.collect() print(gc.mem_free()) At the time of this writing, the CPX has about 17 KB of RAM and the CPB has around 140 KB of RAM. Note that RAM is different than flash. The CPX\/CPB can hold about 2MB of data but only operate with 17\/140 KB of working memory.  "
},
{
  "id": "installing-modules-2-4",
  "level": "2",
  "url": "installing-modules.html#installing-modules-2-4",
  "type": "Figure",
  "number": "7.2.1",
  "title": "",
  "body": "  CircuitPython Libraries homepage  "
},
{
  "id": "installing-modules-2-7",
  "level": "2",
  "url": "installing-modules.html#installing-modules-2-7",
  "type": "Figure",
  "number": "7.2.2",
  "title": "",
  "body": "  Example boot_out.txt file showing the version of CircuitPython  "
},
{
  "id": "installing-modules-2-12",
  "level": "2",
  "url": "installing-modules.html#installing-modules-2-12",
  "type": "Figure",
  "number": "7.2.3",
  "title": "",
  "body": "  CIRCUITPY Drive showing lib folder  "
},
{
  "id": "installing-modules-3-1",
  "level": "2",
  "url": "installing-modules.html#installing-modules-3-1",
  "type": "Figure",
  "number": "7.2.4",
  "title": "",
  "body": "  Example lib folder on CIRCUITPY drive with modules dragged over from desktop  "
},
{
  "id": "moduleerror",
  "level": "2",
  "url": "installing-modules.html#moduleerror",
  "type": "Figure",
  "number": "7.2.5",
  "title": "",
  "body": "  Example of a missing module error  "
},
{
  ...
  "body": " Reformatting your CPX\/CPB  If your CPX\/CPX is completely bricked you can completely reformat it. In order to do that you simply put the CPX\/CPB into CPLAYBOOT mode. Then drag either the CPX eraser tool or the CPB eraser tool . After that, you can reinstall the CircuitPython UF2 and start fresh (See ) . If you have a linux computer you can also access the REPL of your CPX\/CPB using the commands below sudo screen \/dev\/ttyACM0 import storage storage.erase_filesystem() Then you would proceed with installing the CircuitPython UF2 again and starting fresh. Note that this will erase everything on your CPX\/CPB so make sure you have a backup of your code and files before doing this. If you feel like you've tried everything this troubleshooting guide may have some other steps outlined to help you. However, I have had great experience with the eraser tool above .  "
},
{
  "id": "I2C_scanner",
  "level": "1",
  "url": "I2C_scanner.html",
  "type": "Section",
  "number": "7.4",
  "title": "Using an I2C Scanner",
  "body": " Using an I2C Scanner  If you're having trouble getting an I2C device to work, it's a good idea to run an I2C scanner to make sure you can see the device and that you have the correct address. Here is a simple I2C scanner code that you can run on your CPX\/CPB.  import time import board import busio i2c = busio.I2C(board.SCL, board.SDA) while not i2c.try_lock(): pass try: while True: print(\"I2C addresses found:\", [hex(device_address) for device_address in i2c.scan()]) time.sleep(2)  "
},
{
  "id": "troubleshooting-7",
  "level": "1",
  "url": "troubleshooting-7.html",
  "type": "Section",
  "number": "7.5",
  "title": "Common Mishaps",
  "body": " Common Mishaps   Your CPX is a USB stick that can hold as many CircuitPython files as 2MB will allow but it can only run or execute one CircuitPython script at a time. It will look for code.py and then main.py and that's it.  Make sure you're using Mu in the right Mode (CircuitPython)  Make sure main.py or code.py is on the CIRCUITPY drive and not somewhere on your computer.  Make sure you are editing the right file in Mu. Do you have two versions of main.py or perhaps main.py and code.py?  Do you have boot.py on there when you don't need it?  Are you editing using Thonny or Spyder? You're supposed to use Mu.  Are you editing a file on your computer? Make sure you are writing to the CIRCUITPY drive.   "
},
{
  "id": "troubleshooting-8",
  "level": "1",
  "url": "troubleshooting-8.html",
  "type": "Section",
  "number": "7.6",
  "title": "Extra Help",
  "body": " Extra Help  I have a youtube video where I update the bootloader on a CPB That may help you if you're having trouble following the bootloader tutorial above.  "
},
{
  "id": "led-buttons-3",
  "level": "1",
  ...
  "body": " LED with no Code  For this part we are going to light up the LED without the use of any code on the CPX. First, wire up the circuit with the positive end connected to 5V. This is how my circuit looks. Make sure to use a resistor between 150 and 1000 . An LED does not have that much internal resistance so you need a resistor in series with an LED to reduce the amount of current flowing through the LED or the entire LED will fry. If you use a resistor that has too much impedance the LED just won't turn on because the voltage\/current through the LED will be below the activation voltage of the circuit.     CircuitPlayground Express connected to an LED on a breadboard   One issue that you may run into is not knowing which resistor is which. It's actually possible to ascertain the resistance in the resistor by looking at the color bands of the resistor. It's definitely not something worth memorizing but many color charts exist online for you to look at. I've included one below for your convenience. The first two bands are the first two digits of the resistance value, the third band is the multiplier and the fourth band is the tolerance. For example, if you have a resistor with color bands of red, violet, brown and gold, that would be 2 (red), 7 (violet) and 10 (brown) which is 270 with a tolerance of 5% (gold). If you have a resistor with color bands of brown, black, red and gold, that would be 1 (brown), 0 (black) and 100 (red) which is 1000 with a tolerance of 5% (gold). Note that if you have a 5 band resistor, the first three bands are the first three digits of the resistance value, the fourth band is the multiplier and the fifth band is the tolerance.     Resistor color code chart   Once you have that circuit working, wire up the circuit again with the 3.3V output. Do you notice anything different when you hook up the circuit with different pins? Here's my circuit. Do you notice something different about the intensity of the LED? Why is it different?    CircuitPlayground Express connected to an LED using 3.3V   "
},
{
  "id": "led-buttons-3-3",
  "level": "2",
  "url": "led-buttons-3.html#led-buttons-3-3",
  "type": "Figure",
  "number": "8.1.1",
  "title": "",
...
},
{
  "id": "led-buttons-3-5",
  "level": "2",
  "url": "led-buttons-3.html#led-buttons-3-5",
  "type": "Figure",
  "number": "8.1.2",
  "title": "",
  "body": "  Resistor color code chart  "
},
{
  "id": "led-buttons-3-7",
  "level": "2",
  "url": "led-buttons-3.html#led-buttons-3-7",
  "type": "Figure",
  "number": "8.1.3",
  "title": "",
...
},
{
  "id": "led-buttons-6-5",
  "level": "2",
  "url": "led-buttons-6.html#led-buttons-6-5",
  "type": "Figure",
  "number": "8.4.2",
  "title": "",
...
},
{
  "id": "led-buttons-6-9",
  "level": "2",
  "url": "led-buttons-6.html#led-buttons-6-9",
  "type": "Figure",
  "number": "8.4.3",
  "title": "",
...
},
{
  ...
  "number": "8.5",
  "title": "Extra Help",
  "body": " Extra Help  If you're not familiar with how breadboards work I would recommend watching this video on how breadboards work .  "
},
{
  "id": "led-buttons-8",
  "level": "1",
